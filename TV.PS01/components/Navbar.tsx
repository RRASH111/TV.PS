...
const Navbar = () => {
  ...
  const [showCategoriesMenu, setCategoriesMenu] = useState(false);
  ...
  const toggleCategoriesMenu = useCallback(() => {
    setCategoriesMenu((current) => !current);
  }, []);
  ...
  return (
    <nav className="w-full fixed z-40">
      <div className={`px-4 md:px-16 py-6 flex flex-row items-center transition duration-500 ${showBackground ? 'bg-zinc-900 bg-opacity-90' : ''}`}>
        ...
        <div onClick={toggleCategoriesMenu} className="lg:hidden flex flex-row items-center gap-2 ml-8 cursor-pointer relative">
          <p className="text-white text-sm">Categories</p>
          <ChevronDownIcon className={`w-4 text-white fill-white transition`} />
          <CategoriesMenu visible={showCategoriesMenu} />
        </div>
